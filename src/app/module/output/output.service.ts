...
const createOutput = async (payload: TOutput) => {
  ...
  // const lastEntry = await Output.findOne({ date })
  ...
  if (startOfRange <= date && date <= now) {
    ...
    const existingEntry = await Output.findOne({
    ...
    if (existingEntry) {
      throw new AppError(
        httpStatus.FORBIDDEN,
        'A Output entry already exists for the given date',
      )
    }
...
const getAllOutputDownload = async () => {
  const now = new Date()
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
  const endOfMonth = new Date(
    now.getFullYear(),
  ...
  const data = await Output.find({
    ...
    .select(
      'date cuttingCompleted sewingOutput finishingOutput packingCompleted -_id',
    )

  const result = data.map(item => ({
    Date: format(item.date, 'dd-MM-yyyy'), // Format date as 'YYYY-MM-DD'
    'Cutting Completed': item.cuttingCompleted,
    'Sewing Output': item.sewingOutput,
    'Finishing Output': item.finishingOutput,
    'Packing Completed': item.packingCompleted,
  }))
  return result
}
